...
class CreateFunction extends BaseVisitor
{
    ...
    public function process($node)
    {
        if ($this->hasUnpackBefore($node->args)) {
            $this->storage->critical($node, __class__, "create_function含有变长参数列表，可能有代码注入的隐患");
            return;
        }
        ...
        if ((array_key_exists(0, $node->args) && $this->hasDynamicExpr($node->args[0]->value)) || (array_key_exists(1, $node->args) && $this->hasDynamicExpr($node->args[1]->value))) {
            $this->storage->critical($node, __class__, "create_function参数含有动态变量，可能有代码注入的隐患");
        } else {
            $this->storage->warning($node, __class__, "使用create_function有代码执行的隐患，请使用闭包函数替代create_function");
        }
    }
}
